    ...
    withAuth(async (_req, ctx) => {
      ...
      const updated = await prisma.automationConfig.update({
        ...
        data: {
          ...
          execution: data.execution ?? existing.execution,
          ...
          symbols: data.symbols !== undefined ? data.symbols : existing.symbols,
          ...
          maxDailySpendPct: data.maxDailySpendPct ?? existing.maxDailySpendPct,
          dailyLossLimitPct: data.dailyLossLimitPct ?? existing.dailyLossLimitPct,
          stopOnLoss: data.stopOnLoss ?? existing.stopOnLoss,
