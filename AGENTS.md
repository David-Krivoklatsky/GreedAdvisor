...
- **Autonomous engine**: `@greed-advisor/engine` (PM2 `greed-advisor-engine`, **single fork instance — never cluster**) runs the scheduler loop. In dev, `npm run dev` (turbo) also starts `@greed-advisor/engine:dev` (tsx watch) — set `ENGINE_ENABLED=0` in the root `.env` to disable (default is enabled). Safety: Postgres advisory locks + persisted `nextRunAt`; guardrails in `packages/engine/src/guardrails.ts`; mode gating (`advisory`/`paper`/`live`, live requires `allowLive`); per-bot `symbols[]` + `execution` (`auto` | `approval` — approval leaves signals `pending_approval` for `POST /api/user/signals/[id]/approve` → `approveSignal`), `maxDailySpendPct` (cash/day via `DailyStat.spentToday`) and `stopOnLoss` (auto-disables the bot on the daily-loss limit); fill reconciliation + optional trailing stops (`manageStops`, breakeven then 1×ATR trail via `PATCH /v2/orders`) in `packages/engine/src/positions.ts` + pure logic in `src/trail.ts`; per-run step traces (`AutomationRunStep`, recorded via `src/steps.ts`) surfaced on the dashboard. Env: `ENGINE_ENABLED`, `ENGINE_PAUSED`, `ENGINE_TICK_MS`, `ENGINE_WEBHOOK_SECRET` (auths `POST/GET /api/engine/run` for Vercel cron / cron-job.org), `TELEGRAM_BOT_TOKEN`, `NOTIFICATION_WEBHOOK_URL` (engine POSTs `{type,title,body,payload,timestamp}` for every notification). Earnings-calendar awareness is injected into AI prompts via `MarketDataService.getEarnings` (TwelveData `/earnings`). The standalone entry (`src/index.ts` / `src/cycle-cli.ts`) loads env from the repo root `.env` + `packages/db/.env` via `src/env.ts`; the engine must NOT import `@greed-advisor/auth` (throws at import without `JWT_SECRET`).
- **API keys are encrypted at rest**: `@greed-advisor/crypto` AES-256-GCM with `ENCRYPTION_KEY`. Stored values are `enc:v1:...`; `decryptSecret` passes legacy plaintext through unchanged. Keys are encrypted on write (key CRUD routes) and decrypted at the point of use (`packages/trading` bindings, AI/market-data routes, engine). Existing plaintext keys were migrated with `npm run db:encrypt-keys`. To rotate the key: set `NEW_ENCRYPTION_KEY` and run `npm run db:rotate-keys` (re-encrypts all stored keys), then update `ENCRYPTION_KEY`. **Deployments (Vercel/CI) must store `ENCRYPTION_KEY`, `JWT_SECRET` and `DATABASE_URL` as encrypted Secrets, not plain Environment Variables.** Rotating `JWT_SECRET` signs every user out. Rate limiting is in-memory (100 req / 15 min per IP) and now also covers order/AI/automation/key-creation endpoints.
- **Bot markets & strategies**: `AutomationConfig.market` (`us`/`eu`/`crypto`) drives the market-hours gate (crypto = 24/7, EU ≈ CET hours); `strategy` (`momentum`/`trend`/`mean_reversion`/`breakout`/`scalp`/`swing`) selects the analysis timeframe + AI report type. Universe sources: the bot's own `symbols[]`, the user's watchlist, and/or `movers` (auto opportunity hunting via Alpaca movers/most-active, crypto movers for crypto bots).
